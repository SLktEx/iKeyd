...
public sealed class WindowsClipboardPicker : IClipboardPicker
{
    ...
    private static int? PickOnStaThread(IReadOnlyList<string> items)
    {
        using var form = new ClipboardPickerForm(items);
    ...
    private sealed class ClipboardPickerForm : Form
    {
        ...
        public ClipboardPickerForm(IReadOnlyList<string> items)
        {
            ...
            for (var index = 0; index < items.Count; index++)
                _list.Items.Add(new PickerItem(index, BuildPreview(items[index])));
