...
/// <summary>
/// Bounded, non-blocking producer boundary for command actions. Keyboard-path
/// callers use TryEnqueue; one background worker owns Process execution.
/// </summary>
public sealed class WindowsCommandActionQueue : ICommandActionQueue
{
    ...
    public bool TryEnqueue(CommandRequest request)
    {
        ...
        return !_disposed && _channel.Writer.TryWrite(request);
    }
    ...
    private void PublishCompleted(CommandResult result)
    {
        ...
        foreach (Action<CommandResult> handler in handlers.GetInvocationList())
        {
            ...
            {
                // A diagnostic subscriber must never terminate the worker.
            }
        }
    }
