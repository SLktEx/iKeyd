...
/// <summary>
/// Converts a digital direction vector into a smooth two-dimensional virtual
/// stick and integrates the resulting velocity into relative pointer deltas.
/// The engine has no OS or timer dependency: callers provide elapsed time.
/// </summary>
public sealed class VirtualPointerMotionEngine
{
    ...
    public PointerMotionDelta Step(double elapsedSeconds, double pixelsPerSecond)
    {
        ...
        if (IsIdle)
        {
            // Fractional motion from a completed gesture must not leak into the
            // next unrelated gesture.
            _remainderX = 0;
            _remainderY = 0;
        }
    ...
    private double AdvanceAxis(double current, double target, double elapsedSeconds)
    {
        ...
        // Exponential convergence is independent of timer frequency and behaves
        // like an analog stick moving toward a requested position rather than a
        // hold-duration acceleration curve.
