...
public sealed class LinuxUInputDevice : ILinuxVirtualInput, IDisposable
{
    ...
    public void SendText(string text)
    {
        ...
        foreach (var character in text)
        {
            ...
            lock (_gate)
            {
                if (shift) WriteKeyWithoutSync(LinuxInputCodes.KeyLeftShift, 1);
                WriteKeyWithoutSync(code, 1);
                Sync();
                WriteKeyWithoutSync(code, 0);
                if (shift) WriteKeyWithoutSync(LinuxInputCodes.KeyLeftShift, 0);
                Sync();
            }
        }
    }
    ...
    public void MovePointerBy(int deltaX, int deltaY)
    {
        ...
        lock (_gate)
        {
            if (deltaX != 0) LinuxNative.WriteInputEvent(_fd, LinuxInputCodes.EvRel, LinuxInputCodes.RelX, deltaX);
            if (deltaY != 0) LinuxNative.WriteInputEvent(_fd, LinuxInputCodes.EvRel, LinuxInputCodes.RelY, deltaY);
            Sync();
        }
    }

    public void SetMouseButton(ushort buttonCode, bool down) => EmitKeyCode(buttonCode, down ? 1 : 0);
    public void ClickMouseButton(ushort buttonCode) { EmitKeyCode(buttonCode, 1); EmitKeyCode(buttonCode, 0); }
    ...
    public void SendMediaKey(ushort keyCode) { EmitKeyCode(keyCode, 1); EmitKeyCode(keyCode, 0); }
    ...
    public void Dispose()
    {
        lock (_gate)
        {
            ...
            if (_fd >= 0)
            {
                try { LinuxNative.IoctlNoArg(_fd, LinuxNative.UiDevDestroy, "UI_DEV_DESTROY"); } catch { }
                LinuxNative.Close(_fd);
                _fd = -1;
            }
        }
    ...
    private void WriteKeyWithoutSync(ushort code, int value) => LinuxNative.WriteInputEvent(_fd, LinuxInputCodes.EvKey, code, value);
    private void Sync() => LinuxNative.WriteInputEvent(_fd, LinuxInputCodes.EvSyn, LinuxInputCodes.SynReport, 0);
