...
public sealed class LinuxEvdevKeyboardSource : IKeyboardInputSource, IDisposable
{
    ...
    private readonly LinuxUInputDevice _passThroughOutput;
    ...
    public LinuxEvdevKeyboardSource(
        ...
        LinuxUInputDevice passThroughOutput,
    ...
    public void Start(IKeyboardEventHandler handler)
    {
        ...
        lock (_gate)
        {
            ...
            if (_cancellation is not null)
                throw new InvalidOperationException("evdev keyboard source is already running.");
            if (_devicePaths.Count == 0)
                throw new InvalidOperationException("No evdev keyboard devices were configured.");
            ...
            try
            {
                foreach (var path in _devicePaths)
                {
                    ...
                    {
                        if (_grab)
                            LinuxNative.IoctlInt(fd, LinuxNative.EviocGrab, 1, $"EVIOCGRAB('{path}')");
    ...
    public void Stop()
    {
        ...
        {
            if (reader.Thread is { } thread && !ReferenceEquals(thread, Thread.CurrentThread))
                thread.Join();
        }
    ...
    private void ReadLoop(DeviceReader reader, CancellationToken cancellationToken)
    {
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    if (error is LinuxNative.EAgain or LinuxNative.EIntr)
                    {
                        Thread.Sleep(2);
                        continue;
                    }
                    if (error == 0 || error == LinuxNative.EBadF || cancellationToken.IsCancellationRequested)
                        break;
                    throw LinuxNative.Error($"read('{reader.Path}') failed");
                }

                if (inputEvent.Type != LinuxInputCodes.EvKey || inputEvent.Value is < 0 or > 2)
                    continue;
    ...
    private void HandleKeyEvent(LinuxInputEvent inputEvent)
    {
        ...
        if (_keyMap.TryFromEvdev(inputEvent.Code, out var key))
        {
            ...
            {
                // A remapper must fail open: never leave a grabbed keyboard unusable
                // merely because user/core dispatch code threw an exception.
    ...
    private void CleanupReaders()
    {
        foreach (var reader in _readers)
        {
            if (_grab)
            {
                try
                {
                    LinuxNative.IoctlInt(reader.Fd, LinuxNative.EviocGrab, 0, $"EVIOCGRAB release('{reader.Path}')");
                }
                catch
                {
                    // Closing the descriptor releases the grab as well.
                }
            }
