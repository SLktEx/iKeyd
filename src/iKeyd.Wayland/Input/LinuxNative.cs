...
internal static class LinuxNative
{
    ...
    public static void SetupUInputDevice(int fd, string name)
    {
        ...
        // struct input_id: BUS_USB, vendor/product/version. Values only identify the virtual device.
    ...
    private static void EnsureLinux64()
    {
        if (!OperatingSystem.IsLinux())
            throw new PlatformNotSupportedException("The Wayland evdev/uinput backend requires Linux.");
        if (IntPtr.Size != 8)
    ...
    private static class NativeMethods
    {
        ...
        [DllImport("libc", SetLastError = true)]
        public static extern nint read(int fd, [Out] byte[] buffer, nuint count);

        [DllImport("libc", SetLastError = true)]
        public static extern nint write(int fd, byte[] buffer, nuint count);

        [DllImport("libc", SetLastError = true)]
        public static extern int ioctl(int fd, nuint request, nuint argument);
    }
}

internal readonly record struct LinuxInputEvent(
    long Seconds,
    long Microseconds,
    ushort Type,
    ushort Code,
