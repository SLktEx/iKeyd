...
try
{
    ...
    if (checkAgainstPath is not null)
    {
        ...
        var actualNode = JsonNode.Parse(json) ?? throw new InvalidDataException("Generated profile JSON is empty.");
        var expectedNode = JsonNode.Parse(File.ReadAllText(checkAgainstPath)) ?? throw new InvalidDataException("Canonical profile JSON is empty.");
...
{
    Console.Error.WriteLine("Usage: iKeyd.ProfileCompiler <profile.json|profile.ikeyd> <GeneratedProfile.g.cs> [--emit-json <generated.json>] [--check-against <canonical.json>]");
}
...
internal static class ProfileCompiler
{
    ...
    public static string Compile(string json)
    {
        ...
        if (!modeNames.Any(name => string.Equals(name, "S", StringComparison.OrdinalIgnoreCase)) || !modeNames.Any(name => string.Equals(name, "K", StringComparison.OrdinalIgnoreCase)))
        ...
        if (root.TryGetProperty("hotkeys", out var hotkeysElement))
        {
            ...
            foreach (var item in hotkeysElement.EnumerateArray())
            {
                ...
                var trigger = item.GetProperty("trigger").GetString() ?? throw new InvalidDataException("A hotkey trigger must be a string.");
                var action = item.GetProperty("action").GetString() ?? throw new InvalidDataException("A hotkey action must be a string.");
    ...
    private static string BehaviorActionExpression(KeyBehaviorAction action)
        => action.Kind switch
        {
            ...
            KeyBehaviorActionKind.Window or
            KeyBehaviorActionKind.Clipboard or
    ...
    {
        if (!TryGetPropertyIgnoreCase(singleRoot, mode, out var singlesElement)) throw new InvalidDataException($"singleStroke.{mode} is missing from the profile.");
        if (!TryGetPropertyIgnoreCase(chordRoot, mode, out var chordsElement)) throw new InvalidDataException($"chords.{mode} is missing from the profile.");
        if (singlesElement.ValueKind != JsonValueKind.Object) throw new InvalidDataException($"singleStroke.{mode} must be an object.");
        if (chordsElement.ValueKind != JsonValueKind.Array) throw new InvalidDataException($"chords.{mode} must be an array.");
    ...
    private static void AddDistinctModeNames(List<string> names, JsonElement root)
    {
        foreach (var property in root.EnumerateObject())
            if (!names.Any(name => string.Equals(name, property.Name, StringComparison.OrdinalIgnoreCase))) names.Add(property.Name);
    }
    ...
    private static string Literal(string value)
    {
        ...
        foreach (var character in value)
        {
            switch (character)
            {
                ...
                default:
                    if (char.IsControl(character)) builder.Append("\\u").Append(((int)character).ToString("x4"));
