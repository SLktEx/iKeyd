...
legacy = "tests/iKeyd.Windows.Tests/LegacySendEventScenarioRunner.cs"
replace_once(
    legacy,
...
if hosted_text.count(method_marker) != 1:
    raise SystemExit(f"{hosted}: method insertion marker mismatch")
...
hosted_path.write_text(hosted_text.replace(method_marker, method + method_marker, 1), encoding="utf-8")
...
if hosted_text.count(helper_marker) != 1:
    raise SystemExit(f"{hosted}: helper insertion marker mismatch")
hosted_path.write_text(hosted_text.replace(helper_marker, helper + helper_marker, 1), encoding="utf-8")

Path("tests/iKeyd.Windows.Tests/Issue57RuntimeCompatibilityTests.cs").write_text(
    r'''using iKeyd.Compatibility.Tests;
