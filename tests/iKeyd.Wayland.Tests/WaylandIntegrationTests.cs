...
public sealed class WaylandIntegrationTests
{
    ...
    {
        if (Environment.GetEnvironmentVariable("IKEYD_WAYLAND_INTEGRATION") != "1") return;
    ...
    {
        if (Environment.GetEnvironmentVariable("IKEYD_UINPUT_INTEGRATION") != "1") return;
        Assert.True(OperatingSystem.IsLinux());
        ...
        device.ClickMouseButton(LinuxInputCodes.BtnLeft);
        device.ScrollVertical(1);
        device.SendMediaKey(LinuxInputCodes.KeyPlayPause);
