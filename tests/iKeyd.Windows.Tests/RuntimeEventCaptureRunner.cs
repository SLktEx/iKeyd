...
public sealed class RuntimeEventCaptureRunner : ICompatibilityScenarioRunner
{
    ...
    public RuntimeEventCaptureRunner(ICompatibilityScenarioRunner inner) => _inner = inner ?? throw new ArgumentNullException(nameof(inner));
    ...
    private sealed class InjectedEventCapture : IDisposable
    {
        ...
        public InjectedEventCapture(nuint foreignMarker) { _foreignMarker = foreignMarker; _hookProc = HookCallback; }
        public IReadOnlyList<ObservedKeyEvent> Events { get { lock (_gate) return _events.ToArray(); } }

        public void Start()
        {
            _thread = new Thread(ThreadMain) { IsBackground = true, Name = "iKeyd.Legacy.RuntimeEventCapture" };
            _thread.Start(); _started.Wait();
            if (_startError is not null) throw new InvalidOperationException("Could not start legacy runtime event capture.", _startError);
        }
        ...
        public void Dispose()
        {
            ...
            if (thread is not null)
            {
                ...
                if (!ReferenceEquals(Thread.CurrentThread, thread)) thread.Join(TimeSpan.FromSeconds(5));
            }
            _started.Dispose();
        }
        ...
        private void ThreadMain()
        {
            try
            {
                ...
                if (_hook == 0) throw new Win32Exception(Marshal.GetLastWin32Error(), "Could not install runtime event capture hook.");
                _started.Set();
                while (true)
                {
                    var result = NativeMethods.GetMessageW(out var message, 0, 0, 0);
                    if (result == 0) break;
                    if (result == -1) throw new Win32Exception(Marshal.GetLastWin32Error(), "Runtime event capture message loop failed.");
                    NativeMethods.TranslateMessage(ref message); NativeMethods.DispatchMessageW(ref message);
                }
            }
            catch (Exception error) { _startError = error; _started.Set(); }
            finally { if (_hook != 0) NativeMethods.UnhookWindowsHookEx(_hook); _hook = 0; _threadId = 0; }
        }

        private nint HookCallback(int code, nuint wParam, nint lParam)
        {
            var down = wParam == (nuint)WmKeyDown || wParam == (nuint)WmSysKeyDown;
            var up = wParam == (nuint)WmKeyUp || wParam == (nuint)WmSysKeyUp;
            if (code >= 0 && (down || up))
            {
                var native = Marshal.PtrToStructure<KbdLlHookStruct>(lParam);
                if ((native.Flags & LlkhfInjected) != 0 && native.ExtraInfo != _foreignMarker)
                {
                    lock (_gate) _events.Add(new ObservedKeyEvent { Kind = down ? "keyDown" : "keyUp", Key = ScenarioKeyboard.ResolveName((ushort)native.VirtualKey) });
                }
            }
