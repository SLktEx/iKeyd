...
internal static class ScenarioKeyboard
{
    public static ushort ResolveVirtualKey(string key)
    {
        ...
        if (normalized.Length > 1 && normalized[0] == 'F' &&
            int.TryParse(normalized[1..], out var function) && function is >= 1 and <= 12)
            return (ushort)(0x70 + function - 1);

        return normalized switch
        {
            "BS" or "BACKSPACE" => 0x08,
            "TAB" => 0x09,
            ...
            "KANA" => 0x15,
            "ESC" or "ESCAPE" => 0x1B,
            "CONVERT" or "HENKAN" => 0x1C,
            "NONCONVERT" or "MUHENKAN" => 0x1D,
            "SPACE" => 0x20,
            "PGUP" or "PAGEUP" => 0x21,
