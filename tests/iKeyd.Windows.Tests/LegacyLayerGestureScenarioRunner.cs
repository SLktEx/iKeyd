...
/// <summary>
/// Runs complete physical layer gestures against the pinned legacy process.
/// Physical test input is marked through dwExtraInfo and excluded from capture;
/// only key events injected by hotkeySKG are returned.
/// </summary>
internal sealed class LegacyLayerGestureScenarioRunner : ICompatibilityScenarioRunner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!OperatingSystem.IsWindows())
            throw new PlatformNotSupportedException("Layer gesture differential requires Windows.");
        ...
        try
        {
            ...
            // Enter T mode while preserving the S keymap, same as the existing
            // hosted legacy differential. Do this before capture so bootstrap
            // control input and its release effects cannot pollute the gesture.
    ...
    private sealed class InjectedCapture : IDisposable
    {
        ...
        public void Start()
        {
            ...
            {
                IsBackground = true,
                Name = "iKeyd.LayerGestureCapture"
            };
            _thread.Start();
            _started.Wait();
