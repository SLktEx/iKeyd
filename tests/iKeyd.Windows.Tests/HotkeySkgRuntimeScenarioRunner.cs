...
public sealed class HotkeySkgRuntimeScenarioRunner : ICompatibilityScenarioRunner
{
    ...
    public Task<ScenarioRunResult> RunAsync(CompatibilityScenario scenario, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(scenario);
        cancellationToken.ThrowIfCancellationRequested();
        if (!OperatingSystem.IsWindows()) throw new PlatformNotSupportedException("The hotkeySKG runtime scenario runner requires Windows.");
        if (scenario.InitialState.Modifiers.Count != 0) throw new NotSupportedException("Runtime scenarios must express modifiers as explicit input events.");
        ...
        using var runtime = new IKeydRuntimeHandler(configuration, new HostedKanaInputMethod(), keyboardState, send, desktop);
        ...
        foreach (var input in scenario.Input)
        {
            ...
            var kind = string.Equals(input.Kind, "keyUp", StringComparison.OrdinalIgnoreCase) ? KeyEventKind.Up : KeyEventKind.Down;
            var keyboardEvent = new KeyboardEvent(new KeyboardKey(virtualKey, 0, ScenarioKeyboard.IsExtended(virtualKey)), kind, KeyEventOrigin.Physical, input.AtMs);
    ...
    private sealed class HostedKanaInputMethod : IInputMethod { public bool IsKanaInputActive() => true; }
    ...
    private sealed class RecordingKeyboardOutput : IKeyboardOutput
    {
        ...
        public void SendKey(KeyboardKey key, KeyEventKind kind) => Events.Add(new ObservedKeyEvent { Kind = kind == KeyEventKind.Down ? "keyDown" : "keyUp", Key = ScenarioKeyboard.ResolveName(key.VirtualKey) });
    ...
    private sealed class NoOpDesktopBackend : IDesktopBackend
    {
        ...
        public void SetMouseButton(DesktopMouseButton button, bool down) { if (down) _buttons.Add(button); else _buttons.Remove(button); }
