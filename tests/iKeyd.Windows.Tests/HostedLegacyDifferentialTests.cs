...
public sealed class HostedLegacyDifferentialTests
{
    ...
    public async Task Tagged_scenarios_match_the_real_legacy_exe_on_hosted_windows()
    {
        ...
        if (!baseLegacyRunner.IsAvailable) return;
        ...
        foreach (var scenario in scenarios)
        {
            try
            {
                ...
                ICompatibilityScenarioRunner legacyRunner = runtimeScenario
                    ? new RuntimeEventCaptureRunner(new RuntimeInputInjectionRunner(new HostedTModeLegacyRunner()))
                    : new HostedTModeLegacyRunner();
                var report = await LegacyDifferentialComparison.RunAsync(scenario, iKeydRunner, legacyRunner);
            ...
            {
                failures.Add($"Hosted differential runner failed for scenario '{scenario.Id}': {error.GetType().Name}: {error.Message}");
            }
        }
    ...
    public void Hosted_adapter_bootstraps_inner_runner_in_S_mode_and_disables_IME_dependency()
    {
        var scenario = new CompatibilityScenario
        {
            ...
            Input = [new ScenarioInputEvent { AtMs = 0, Kind = "keyDown", Key = "K" }, new ScenarioInputEvent { AtMs = 10, Kind = "keyDown", Key = "Q" }],
            Expected = new ScenarioExpected { Text = "ti" }
        };
        var adapted = HostedTModeLegacyRunner.PrepareScenario(scenario);
        Assert.Equal("S", adapted.InitialState.Mode); Assert.Equal("off", adapted.InitialState.Ime);
        Assert.Equal(500, adapted.Input[0].AtMs); Assert.Equal(510, adapted.Input[1].AtMs);
        Assert.Equal("K", scenario.InitialState.Mode); Assert.Equal("on", scenario.InitialState.Ime); Assert.Equal(0, scenario.Input[0].AtMs);
    }
