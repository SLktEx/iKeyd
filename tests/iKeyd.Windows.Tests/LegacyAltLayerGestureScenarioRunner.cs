...
/// <summary>
/// Physical legacy-process oracle for layer gestures that include Alt.
/// Scenario input is injected with a private marker and excluded from capture;
/// only output injected by hotkeySKG is observed.
/// </summary>
internal sealed class LegacyAltLayerGestureScenarioRunner : ICompatibilityScenarioRunner
{
    ...
    public string Name => "hotkeySKG Alt layer-gesture oracle";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!OperatingSystem.IsWindows())
            throw new PlatformNotSupportedException("Alt layer gesture differential requires Windows.");
        ...
        try
        {
            ...
            // Enter T mode while preserving the S keymap, matching the other
            // hosted legacy differentials. Bootstrap happens before capture.
            ...
            return new ScenarioRunResult
            {
                ...
                {
                    ["scope"] = "legacy-alt-layer-gesture"
                }
            };
        }
        finally
    ...
    private static (byte VirtualKey, byte ScanCode) Resolve(string key)
    {
        ...
        return normalized switch
        {
            ...
            _ => throw new NotSupportedException($"Unsupported Alt layer-gesture input '{key}'.")
        };
    }
    ...
    private sealed class InjectedCapture : IDisposable
    {
        ...
        public void Start()
        {
            ...
            {
                IsBackground = true,
                Name = "iKeyd.AltLayerGestureCapture"
            };
            _thread.Start();
            _started.Wait();
            if (_startError is not null)
                throw new InvalidOperationException("Could not start Alt layer-gesture capture.", _startError);
        }
