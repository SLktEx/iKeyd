...
public sealed class VirtualPointerMotionEngineTests
{
    ...
    [Fact]
    public void Opposing_digital_inputs_can_cancel_to_zero()
    {
        var engine = ImmediateLinear();
        engine.SetDirection(-1 + 1, 0);

        var delta = engine.Step(0.1, 1000);
    ...
    [Fact]
    public void Integration_is_based_on_elapsed_time_not_tick_count()
    {
    ...
    [Fact]
    public void Press_and_release_are_smoothed_without_long_lived_inertia()
    {
        ...
        var pressedAxis = engine.AxisX;
        Assert.InRange(pressedAxis, 0.62, 0.64);
    ...
    [Fact]
    public void Reversing_direction_crosses_through_the_smoothed_axis()
    {
        var engine = new VirtualPointerMotionEngine(new VirtualPointerMotionOptions
        {
            PressTimeConstantSeconds = 0.045,
